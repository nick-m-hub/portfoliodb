...
export async function getAllAllocations() {
  const [
    ...
    (async () => {
      console.time('db:getAllAllocations:allocations');
      const result = await supabase.from('allocations').select('*');
      console.timeEnd('db:getAllAllocations:allocations');
      return result;
    })(),
    (async () => {
      console.time('db:getAllAllocations:asset_classes');
      const result = await supabase.from('asset_classes').select('asset_class, default_color');
      console.timeEnd('db:getAllAllocations:asset_classes');
      return result;
    })(),
  ]);
...
// Portfolio names/slugs only change when a new portfolio is added, so this is
// cached indefinitely. IMPORTANT (corrected July 2026): a Vercel redeploy does
// NOT clear this — Vercel's Data Cache backing unstable_cache is durable and
// persists across deployments regardless of code changes, unlike a
// self-hosted Next.js server's in-memory cache. Discovered when a newly added
// portfolio was live on /database (getPortfolios(), uncached) but missing
// from the Builder/FI Calculator/Navbar search (all read getPortfolioNames())
// after multiple redeploys. `tags: ['portfolio-names']` below enables a
// future on-demand `revalidateTag('portfolio-names')` call; there is no
// route wired up to call it yet, so the only way to force-refresh today is
// bumping the key array (e.g. 'portfolio-names-v2') — see New Portfolio
// Checklist. No `revalidate` option here: any numeric value gets adopted as
// the route-level ISR revalidate for every page that renders the root
// layout, which would silently turn fully-static pages (and /leaderboard,
// which explicitly must not use ISR) into ISR pages.
export const getPortfolioNames = unstable_cache(
  async () => {
    console.time('db:getPortfolioNames:query');
    const { data, error } = await supabase
      .from('portfolios')
      .select('name, slug, kofi_link')
      .order('name', { ascending: true });
    console.timeEnd('db:getPortfolioNames:query');
...
// Group raw tactical_monthly_holdings rows into per-portfolio signal objects.
// Converts weight from the stored decimal fraction (0–1) to a percentage (0–100).
// Shared by getCurrentSignals() and /api/builder-holdings (CR-12).
export function groupTacticalHoldings(rows, nameBySlug) {
  const bySlug = {};
...
export async function getCurrentSignals() {
  ...
  console.time('db:getCurrentSignals:step1');
  const [
  ...
  console.timeEnd('db:getCurrentSignals:step1');
  ...
  console.time('db:getCurrentSignals:step2b');
  ...
  console.timeEnd('db:getCurrentSignals:step2b');
  ...
  return groupTacticalHoldings(data, nameBySlug);
}
