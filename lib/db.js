...
export async function getCurrentSignals() {
  ...
  // Step 2 — get all tactical holdings for those portfolios, newest first
  const { data, error } = await supabase
  ...
  // Step 3 — keep only the most recent date per portfolio
  const latestBySlug = {};
  for (const row of data ?? []) {
    const slug = row.portfolio_slug;
    if (!latestBySlug[slug]) {
      // First occurrence for this slug = most recent date (data ordered date DESC)
      latestBySlug[slug] = {
        slug,
    ...
    if (row.date === latestBySlug[slug].date) {
