...
- [ ] **Correlation matrix (`/tools/correlation`)** — A full 70×70 (or filtered subset) pairwise correlation heatmap between all portfolio return series. Color-coded: dark green = low correlation (diversifying), dark red = high correlation (redundant). Answers "if I hold Golden Butterfly alongside All-Weather, how redundant is that?" Computable entirely from existing `monthly_returns` using Pearson correlation on the overlapping date range for each pair. Implementation: server-computed correlation matrix passed to a client heatmap component (custom SVG or a lightweight library). Filtering by category or strategy tag would be a useful V1 feature. Add to Tools dropdown. Low-medium effort.
