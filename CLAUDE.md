...
**`blog-drafts/` is gitignored (Aug 2026).** Because the Supabase `blog_posts` row is the source of truth and local copies drift (see above), the directory was added to `.gitignore` so new drafts stop showing up as untracked noise. Consequences: a new draft file is **local-only** — it won't be committed, won't sync to another machine, and is lost if the working copy is wiped. Two drafts committed before the ignore rule (`bogleheads-three-fund-vs-four-fund-portfolio.md`, `may-2026-portfolio-performance-review.md`) remain tracked, since `.gitignore` doesn't untrack existing files. To version a new draft deliberately, use `git add -f blog-drafts/[slug].md`.
...
portfoliodb/
  app/
    ...
    account/
      ...
      loading.js                     # Instant loading skeleton (July 2026) — Next.js segment file shown immediately on navigation to /account while the force-dynamic page renders (removes the "dead click" feel on cold starts). Mirrors the page layout (header + Plan card + Saved Mixes + Current Signals grid); purely presentational, no data/auth
    auth/
      callback/
  ...
  components/
    ...
    EfficientFrontierChart.jsx       # Recharts ScatterChart for the Builder's Efficient Frontier section (client) — Pareto frontier line + orange "your mix" dot, styled to match PortfolioMapClient's frontier; lazy-loaded via next/dynamic (ssr: false) in BuilderClient.jsx
    ...
    SavedMixList.jsx                 # Client component — displays saved mixes with inline delete confirmation (no window.confirm); buildLoadUrl() generates /builder?mix=slug:weight,... URLs; empty state + Builder tier (x/3) counter with upgrade link; shows blended holdings per mix card (Signals tier = full blend incl. tactical; others = buy-and-hold portion only, unblurred, + "tactical holdings hidden" lock note — CR-1). Slimmed July 2026 (INP): takes only `initialMixes` + `tier`; each mix's pills + blended holdings are precomputed server-side in account/page.js, so the ~100KB allAllocations payload and blendHoldings() no longer ship to / run on the client
  ...
  app/
    leaderboard/
      page.js                        # Strategy Leaderboard (server, fully static SSG) — uses anon supabase client (NOT createServerSupabaseClient). No revalidate — page rebuilds with each Vercel deploy, which aligns with the monthly data update cycle. IMPORTANT: do NOT add revalidate or call cookies() here; either opts the page into ISR reads. Public data only — no auth needed. Fetches portfolio_stats selecting cagr_1yr/cagr_3yr/cagr_10yr/ytd_return/sharpe_ratio/rolling_1yr_avg/rolling_3yr_avg/rolling_10yr_avg; passes to LeaderboardClient
      LeaderboardClient.jsx          # Client component — 8 tabs: YTD/1-Year/3-Year/10-Year/Sharpe (best-first, 🥇🥈🥉 medals) plus 1Y/3Y/10Y vs. Avg (Aug 2026 — site-wide extension of the portfolio detail page's "Performance vs. Own History" gauge, see that CLAUDE.md entry). The "vs. Avg" tabs compute `cagr_Xyr − rolling_Xyr_avg` client-side via a per-tab `compute(p)` fn (no new SQL — both source columns already existed) and sort **ascending** (worst-laggard-first) via a per-tab `sortAsc` flag — the opposite of the other 5 tabs — since the useful view for a watchlist is "who needs attention," not scrolling to the bottom of 78 rows. Top-3 rows on ascending tabs get a `warning` Material Symbol + soft red row tint (`bg-[#fdf2f2]`) instead of medals — gold medals on the worst-performing rows would read backwards. `fmt()` handles both `%` and `pp` (percentage-point) suffixes with the same sign-based red/green coloring. Category badges; links to portfolio detail pages.
    changelog/
...
- Client component — props (slimmed July 2026 for INP): `initialMixes` (array) + `tier` ('builder' | 'signals') **only**. Each `initialMixes` item is precomputed server-side in `app/account/page.js` and carries everything the UI needs: `{ id, name, created_at, selections: [{slug, weight}], pills: [{slug, weight, name}], blended: { hasTactical, holdings } }`. The component no longer receives `allAllocations`/`allSignals`/`tacticalSlugs`/`portfolioNames` and no longer runs `blendHoldings()` or builds any lookup maps — the ~100KB allocations payload + blend math were moved server-side (see the account page.js note below). `pills[].name` is resolved server-side from `getPortfolioNames()` (CR-10: saved selections only store `{slug, weight}`, so names self-heal on renames).
...
- **Blended holdings** shown inside each mix card below the weight pills — computed by `blendHoldings()` (portfolio weight × holding weight, summed by ticker), now run **server-side** in `app/account/page.js` and passed in as `mix.blended`. **CR-1 (July 2026):** Signals tier sees the full blend (incl. tactical); non-Signals users only ever receive the buy-and-hold portion (public allocation data, shown unblurred) plus a "this mix includes tactical holdings" lock note — no real signal data reaches their DOM. This holds unchanged after the server-side move: `blendHoldings` only reads tactical tickers from the `signals` array, which is `[]` for non-members, so a tactical portfolio contributes no tickers but still sets `hasTactical: true` (drives the lock note). The `BlendedHoldings` sub-component just renders the precomputed `{ hasTactical, holdings }` — it no longer computes anything.
...
- Fetches subscription + saved mixes + `getAllAllocations()` (public) + `getPortfolioNames()` (cached; supplies kofi_link → tacticalSlugs) + signals in a single `Promise.all`. **CR-1 (July 2026): `getCurrentSignals()` only runs for Signals members** — non-members get `signals = []` and never receive real signal data in the page payload. **Signals-overlap refactor (July 2026):** the signals fetch is chained off the subscription promise (`subscriptionPromise.then(sub => tierFromSubscription(sub)==='signals' ? getCurrentSignals() : [])`) and included in the same `Promise.all`, so it starts the moment the subscription check resolves and overlaps the rest of the batch instead of running as a sequential step after it. Removes one round-trip layer from the warm critical path (~300ms when the batch is slow). Diagnosed July 18 via temporary `console.time` instrumentation (since removed): warm render ≈ 370ms; the ~4s Speed Insights p75 is cold starts, not warm query time (see Performance Notes).
...
- Saved Mixes section: always renders `<SavedMixList>` (every signed-in user has at least Builder access — the old "locked, no tier" empty state was removed June 2026); shows `(x/3)` counter when `tier === 'builder'`. **INP refactor (July 2026):** the page precomputes a `mixCards` array server-side — for each saved mix it builds the display `pills` (weight + name via `nameBySlug`) and the `blended` holdings via `blendHoldings()` (using server-side `allocBySlug`/`signalBySlug`/`tacticalSet` maps), then passes only `initialMixes={mixCards}` + `tier` to `SavedMixList`. Previously it passed the full `allAllocations` (~566 rows / ~100KB), `allSignals`, `tacticalSlugs`, and `portfolioNames`, and the client ran `blendHoldings` + built lookup maps on hydration — pure main-thread INP cost to render pills for ≤3 mixes. `getAllAllocations()` is still fetched in the parallel batch (perf-audit 2.2 — gating it would add a waterfall for the primary audience) but is now consumed server-side and never serialized to the browser. CR-1 preserved: non-members' `signals` is `[]`, so `blendHoldings` emits only the public buy-and-hold portion + `hasTactical` flag.
...
- **Auto-allocate dropdown** (visible when `selections.length >= 2 && allDataLoaded && !isLoading`): a compact native `<select>` labelled "Auto-allocate:" with four options — **Equal Weight**, **Risk Parity**, **Max Sharpe**, **Max Sortino**. Applies weights immediately on selection and resets to "Choose…" placeholder (stateless). A single `StatTooltip` ⓘ icon describes all four modes using a JSX fragment with `<br/>` line breaks as the `definition` prop. `riskParityWeights()` — inverse-vol over common date window, rounds to 1 decimal, last slot absorbs remainder, null if < 12 common months. `maxSharpeWeights()` — analytical Markowitz tangency portfolio: tries all 2^n non-empty subsets of assets, solves Σw = μ_excess via Gauss-Jordan elimination for each subset, keeps feasible solutions (all weights ≥ 0), picks the subset with the highest Sharpe. Applies a 5% minimum weight floor post-solve via the shared `applyWeightFloor()` helper (iterative clamp-and-redistribute, extracted July 2026 so `maxSortinoWeights()` can reuse it) so no portfolio in the mix is zeroed out. Falls back silently if < 12 common months or no feasible solution.
- **`maxSortinoWeights()` (July 2026)** — no closed-form long-only solution exists for Sortino (unlike Max Sharpe's covariance-matrix inversion), since downside deviation depends on which specific months are negative in the *blended* series, not a quadratic form of the weights. Instead built on `sampleEfficientPortfolios()`: Monte Carlo samples the weight simplex (equal-weight + each pure single-asset vertex + ~3,000 Dirichlet(1) random draws), computing REAL compounded stats (CAGR, annualized volatility, Sortino) per candidate via the same per-month compounding loop as `computeStats()` — not a mean-variance approximation. Then runs 3 rounds of localized re-sampling (500 samples each, shrinking perturbation radius) around the best-scoring candidate to polish the optimum, and applies `applyWeightFloor()` for the final result. n ≤ 6 keeps this comfortably sub-200ms.
- **Efficient Frontier section (July 2026)** — new full-width gated row after Drawdown + Rolling Returns, before Withdrawal Rates. Reuses the same `sampleEfficientPortfolios()` cloud (uncapped, no `refineFor`) and Pareto-filters it via `computeFrontier()` — the identical greedy sweep algorithm as `PortfolioMapClient.jsx`'s `frontierPoints` (sort by volatility asc / CAGR-desc tiebreak, keep points whose CAGR beats the running max), just applied to a sampled cloud of hypothetical blends of the *currently selected* portfolios instead of a fixed set of real portfolios — this is the one place in the app where a true covariance-style frontier makes sense (see the Portfolio Map's own doc note on why it deliberately avoids one). Rendered via `components/EfficientFrontierChart.jsx` (Recharts `ScatterChart`, dynamically imported `ssr:false`): the frontier line uses the same style tokens as the Map's frontier (`stroke: '#27624a'`, `strokeWidth: 2`, `strokeOpacity: 0.55`, `lineJointType="monotoneX"`), and the user's actual mix is plotted as an orange (`#e67e22`) dot using `stats.annualizedVolatility`/`stats.cagr` directly — matching the Map's "your mix" dot color for a consistent visual language. `frontierData` useMemo is deferred behind `tier` (like `heatmapData`) and keyed on `selectedSlugsKey` (portfolio slugs only, not weights) rather than raw `selections`, so dragging a weight slider moves the current-mix dot without re-running the ~3,000-sample search — only adding/removing a portfolio triggers a recompute.
...
### Performance vs. Own History gauge (Aug 2026)
- Built into the existing "Rolling Returns" section (`id="rolling-returns"`) in `app/portfolios/[slug]/page.js`, above the existing Low/Average/High table — not a separate component or nav section. Motivation: a tactical portfolio's headline stats can look strong in absolute terms while its recent trailing return is quietly underperforming its own long-run rolling-return average — this makes that visible at a glance without a new page. This is the "simple gauge" v1 from the two-tier design; see TASKS.md "Underperformer Watchlist — robust version" for the deferred percentile/z-score/severity-rating follow-up.
- **Data:** three rows — 1-Year, 3-Year, 10-Year — each plotting `cagr_{1yr,3yr,10yr}` (the current trailing window) against `rolling_{1yr,3yr,10yr}_{low,avg,high}` (the full historical distribution of that same rolling window). All six columns already exist on `portfolio_stats`; no new SQL. **5-Year is deliberately excluded from the gauge** (still shown in the table below it) — the view has `rolling_5yr_low/avg/high` but no `cagr_5yr` trailing column to plot as "current," so there's nothing to mark on that row.
- **Why current is always inside [low, high]:** `cagr_Xyr` is computed the same way as the rolling series (calendar window ending at the latest month, ≥ the same minimum-months threshold) — it *is* the most recent rolling window, not a separately-computed figure — so it's mathematically guaranteed to fall within its own `rolling_Xyr_low`/`rolling_Xyr_high` range. Confirmed against `scripts/portfolio_stats_view.sql`.
- **Rendering:** each row is a `relative` track (`h-2 rounded-full bg-surface-container`) with two absolutely-positioned markers — a thin tick at the historical average (`bg-outline`) and a dot at the current value, colored `bg-primary` (green) when current ≥ avg or `bg-error` (red) when below. Position = `((value - low) / (high - low)) * 100`, clamped 0–100 defensively (`clampPct` local helper). Low/high values are printed under the track's left/right ends; a legend below the three rows explains the two dot colors and the avg tick.
- **Reading the three rows together implies trend** without any extra slope computation: if the 1-Year row sits below its avg while 3-Year/10-Year sit above (or vice versa), that ordering itself signals whether recent performance is accelerating or fading relative to the long-term track record — this was the deciding factor in choosing all three timeframes over just 1yr.
- **Null handling:** a row is omitted entirely if any of its four inputs (`current`/`low`/`avg`/`high`) is null (new-ish portfolios without enough history for a given window); the whole gauge block (including the divider before the table) is omitted if all three rows are missing. Verified via a full production build — all 78 portfolio pages prerendered cleanly, including newer/shorter-history ones.
- Pure server-rendered JSX (no client component, no new file) — computed inline in the same IIFE that already builds the table's `rows` array, right below it.
- **"Trending Up/Down/Mixed Signal" pill (Aug 2026 follow-up):** a badge next to the "Rolling Returns" heading that turns the "reading the rows together" inference above into an explicit label, requested by Nick after reviewing the gauge and asking whether trend was actually surfaced anywhere. Computed from the same `gaugeRows` deltas (`current − avg`) already built for the bars — no new data. Walks the deltas in the existing shortest→longest order (1yr→3yr→10yr, whichever windows exist) and requires **every consecutive step to move by more than `TREND_EPS` (1.5pp) in the same direction**: strictly decreasing → "Trending Up" (`trending_up`, green), strictly increasing → "Trending Down" (`trending_down`, red), anything non-monotonic (including "consistently bad but not worsening in order," e.g. a dip-then-partial-recovery shape) → "Mixed Signal" (`trending_flat`, neutral). Needs ≥ 2 gauge rows; omitted entirely otherwise.
  - **Known distribution as of Aug 2026 (checked live against `portfolio_stats`, not a bug):** 57 of 78 portfolios read "Trending Up," 21 "Mixed Signal," 0 "Trending Down." This is the label correctly reflecting an unusually strong trailing 12 months across nearly the whole database (Leaderboard's 1-Year tab topped out around +33%) — when almost every portfolio's `cagr_1yr` sits far above its own `rolling_1yr_avg` while `cagr_10yr` (which bakes in that same strong recent stretch) sits closer to or below its average, the shortest-window delta is mechanically the highest, so "Up" dominates. The zero "Down" count is a side effect of the monotonicity requirement being strict by design: several of the worst 1yr-vs-avg laggards (e.g. `quint-switching-filtered`, `ivy-portfolio-rotation`, `trend-following-bonds-by-paul-novell`) have a 3-year delta that dips *below* both their 1yr and 10yr deltas rather than sitting cleanly between them, which breaks monotonicity and lands them in "Mixed Signal" instead of "Down" even though they're underperforming across the board. **This distribution will shift with market conditions** — it is not hardcoded and needs no manual correction, but don't be surprised if "Down" stays rare; a portfolio needs a clean, uninterrupted worsening from its 10-year to its 1-year standing to qualify, which is a higher bar than merely "underperforming everywhere."
...
**Adding a new tier (future note, July 2026):** `tierFromSubscription()` is the single source of truth for *which* tier a user is, but the *"does this tier unlock signals?"* decision is currently expressed as a scattered inline equality check — `tierFromSubscription(sub) === 'signals'` (the `/account` `signalsPromise` gate, `app/account/page.js`), `plan === 'signals'` (`getEntitledSubscription` preference + `isSignalsEntitled`, `lib/entitlements.js`), and `tier === 'signals'` (client components `SavedMixList.jsx`, `CurrentSignals.jsx`). These do NOT inherit each other. If you add a tier that should ALSO see signals (e.g. a "Signals Plus"), you must broaden *every* one of those checks or it will be silently treated as non-Signals in some places. Recommended at that point: extract one pure predicate, e.g. `subscriptionUnlocksSignals(sub)` / `tierUnlocksSignals(tier)`, into `lib/entitlements.js` and replace the inline `=== 'signals'` sites so it's a one-line change thereafter. Note: the `/account` `Promise.all` structure itself does NOT need changing to add a tier — new per-tier data just follows the same `subscriptionPromise.then(sub => <predicate> ? getX() : [])` chain pattern already used for signals.
...
- Material Symbols font URL uses fixed axis values (`opsz=24,wght=400,FILL=0,GRAD=0`) — do NOT widen these ranges; it caused the font to balloon from ~50 KB to ~3,800 KB and FCP to spike to 21 s. The same URL also ends in **`&display=block` (NOT `swap`)** — see the Aug 2026 CLS entry below before changing it.
...
- **`/account` cold-start diagnosis (July 18, 2026):** Speed Insights showed TTFB p75 ≈ 3.95s. Instrumented every Supabase call on the render path with temporary `console.time` (since removed) and confirmed: a *warm* render is ≈ 370ms (the middleware `auth.getUser` ≈ 230ms is the single biggest warm cost; the queries are 47–71ms uncontended — the 800ms+ figures in one sample were contention artifacts from a concurrent `/builder` burst, not baseline). The ~4s p75 is **cold starts** — `/account` is `force-dynamic` (must show fresh subscription + mixes) AND low-traffic (logged-in only), so its Lambda goes idle and the next visitor pays boot + fresh Supabase TLS setup. Confirmed by a captured cold render (~3.2s server time) where the `unstable_cache`-backed `getPortfolioNames` inner query actually ran (empty cache = fresh instance). Supabase is `us-east-1`, colocated with Vercel `iad1`, so distance is NOT a factor; indexes are NOT the issue (a 30-row `asset_classes` query at 834ms can't be scan time). **Fixes applied:** (1) signals-overlap refactor (see account page notes, ~300ms on slow batches); (2) `app/account/loading.js` instant skeleton — targets Nick's actual complaint (clicking Account gave no feedback during the cold render), does not speed up the data. Fluid Compute confirmed already ON (free platform cold-start optimizations active but insufficient alone for a low-traffic page). Deferred (TASKS.md): Vercel Pro Scale-to-One (~$20/mo, eliminates cold starts — Nick declined paying for now) or a deeper static-shell + client-side data fetch.
...
- **CLS fix — Material Symbols icon font (Aug 2026, site-wide):** Vercel Speed Insights showed poor Cumulative Layout Shift on `/account` (0.24) and `/database` (0.42). One driver was the icon font loading with **`&display=swap`**: before the webfont arrived, every `<span className="material-symbols-outlined">manage_accounts</span>` rendered its **ligature source text** ("manage_accounts", multi-character wide) in the fallback font, then collapsed to a single ~1em glyph on load — reflowing every icon's neighbours, worst on cold-start pages where the font lands late (inside the CLS window). Note this is the *same* raw-text flash the June 2026 FOUC entry above describes — `swap` reintroduced it via the font-display path rather than the stylesheet-loading path. **Two-part fix:** (1) `&display=swap` → **`&display=block`** in `app/layout.tsx` (fallback text stays invisible during load instead of flashing raw ligature text); (2) `.material-symbols-outlined` in `app/globals.css` now reserves a fixed 1em box — `display:inline-block; width:1em; line-height:1; overflow:hidden; white-space:nowrap` — so any pre-load fallback text is clipped to the icon box and **cannot reflow siblings regardless of font timing**. These are the declarations Google's own Material Symbols CSS ships; the site's minimal rule had dropped them. **Do NOT revert either** without re-measuring CLS. Verified: every loaded glyph fits its 1em box exactly (scrollWidth == clientWidth == font-size, zero clipping) — `overflow:hidden` only ever clips the transient fallback text. Icons inside flex/grid containers compute as `display:block` (CSS blockification of flex items) — harmless, the 1em box still applies.
- **CLS fix — `/database` prerendered the grid (Aug 2026):** `/database` had the site's worst CLS (0.42). `DatabaseClient` called `useSearchParams()`, which forces the component behind a Suspense boundary, so Next prerendered **only the spinner fallback** into the static HTML and the full 78-card grid rendered client-side after hydration — a full-page swap on every visit (also delayed LCP and left the grid out of the initial HTML for crawlers). Confirmed against production via `curl`: served HTML contained `progress_activity` and portfolio names appeared only inside the `self.__next_f` RSC flight payload, not rendered DOM. Fixed by replacing `useSearchParams()` with a one-time mount `useEffect` reading `window.location.search` (it only seeded 3 filter states), which removed the Suspense requirement — see the DatabaseClient section above for the full rationale and the "do not reintroduce" warning. Prerendered `database.html` went ~234KB → ~501KB (grid now real DOM); `/database` stays SSG. **General lesson: `useSearchParams()` in a large client component silently costs you the entire server-rendered payload for that subtree.** Audited the rest of the site (Aug 2026) — no other page had this pattern; `/tools/correlation` is a separate, unconfirmed CLS candidate with a different cause (heavy on-mount API fetch; fix is to reserve height, see TASKS 1.3b).
- **INP fix — `/account` saved-mix blend moved server-side (Aug 2026):** Speed Insights showed INP 1,304ms on `/account` (interactivity, distinct from the cold-start TTFB work above). Cause was hydration cost, not the event handlers: the page shipped the **entire `allAllocations` list (566 rows / ~100KB measured)** plus `allSignals` + `portfolioNames` to `SavedMixList`, which parsed it, built lookup maps, and ran `blendHoldings()` **on the client** — just to render pills for ≤3 saved mixes (and 100KB shipped for nothing in the 0-mix case). Fixed by precomputing `mixCards` server-side and passing only `initialMixes` + `tier` (see the SavedMixList + account page sections above). CR-1 is preserved because `blendHoldings` only reads tactical tickers from `signals`, which is `[]` for non-members. Note `/database`'s INP was already fine (72ms) — it isn't `force-dynamic`, so this was an `/account`-specific problem.
